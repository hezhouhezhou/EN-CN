...
def training_loop(tokenized_dataset_index_en, tokenized_dataset_index_zh, train_dict, val_dict, idx_dict, encoder, decoder, criterion, optimizer, opts):
    ...
    for epoch in range(opts.nepochs):
        ...
        input_tensors = [torch.LongTensor(w) for w in tokenized_dataset_index_en]
        target_tensors = [torch.LongTensor(w) for w in tokenized_dataset_index_zh]
        num_tensors = len(input_tensors)
        num_batches = int(np.ceil(num_tensors / float(opts.batch_size)))
...
def main(opts):
    ...
    train_pairs, val_pairs = tokenized_dataset[:num_train], line_pairs[num_train:]
    ...
    if opts.no_attention:
        decoder = models.NoAttentionDecoder(vocab_size=len(vocabulary_zh), hidden_size=opts.hidden_size)
    else:
        decoder = models.AttentionDecoder(vocab_size=len(vocabulary_zh), hidden_size=opts.hidden_size)
    ...
    try:
        training_loop(tokenized_dataset_index_en, tokenized_dataset_index_zh, train_dict, val_dict, idx_dict, encoder, decoder, criterion, optimizer, opts)
...
if __name__ == '__main__':
    ...
    tokenized_dataset_index_en, tokenized_dataset_index_zh = sentencetoindex(tokenized_dataset,word_to_index),sentencetoindex(tokenized_dataset_zh,index_to_zh)
